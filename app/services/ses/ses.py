...
class correlator():
    ...
    def sesinfrastructure_correlate(self):
        ...
        queries.append(
            '''INSERT INTO ids_ses_correlation (SELECT * from temp_ses) ON DUPLICATE KEY UPDATE ids_ses_correlation.description = CONCAT(ids_ses_correlation.description, "\n", VALUES(description))''')
    ...
    def update_httpcnc(self, data):
        ...
        for row in reader:
            try:
                ...
                query = '''INSERT INTO seshttpcnc (sha1, md5, tdstamp, ip, asn, cc, url) VALUES ('%s', '%s', DATE('%s'), INET_ATON('%s'), %s, '%s', '%s') ON DUPLICATE KEY UPDATE tdstamp=tdstamp''' % (
                    sha1, md5, timestamp, ip, asn, cc, url)
                # print "query = %s" % query
                self.cursor.execute(query)
    ...
    def update_malwareflows(self, data):
        ...
        for row in reader:
            try:
                ...
                query = '''INSERT INTO sesmalwareflows (sha1, md5, tdstamp, ip, dport, pr, app, asn, cc, fqdn )  VALUES ('%s', '%s', DATE('%s'), INET_ATON('%s'), %s, %s, '%s', %s, '%s', '%s') ON DUPLICATE KEY UPDATE tdstamp = tdstamp''' % (
                    sha1, md5, tdstamp, ip, dport, pr, app, asn, cc, fqdn)
                # print "query = %s" % query
                self.cursor.execute(query)
    ...
    def update_infrastructure(self, data):
        ...
        for row in reader:
            try:
                ...
                VALUES (%s, INET_ATON('%s'), '%s', '%s', '%s', '%s', '%s', '%s', DATE('%s'), DATE('%s'), '%s') ON DUPLICATE KEY UPDATE updated = updated, confidence = confidence, action = action''' % (
                    asn, ip, protocol, restriction, description, impact, action, confidence, created, updated, ref)
                # print "query = %s" % query
                self.cursor.execute(query)
    ...
    def update_cncip(self, data):
        ...
        for row in reader:
            try:
                ...
                query = '''INSERT INTO sescncip (asn, description, ip, protocol, discovered, expiration, category, comments) VALUES (%s, '%s', INET_ATON('%s'), '%s', DATE('%s'), DATE('%s'), '%s', '%s') ON DUPLICATE KEY UPDATE expiration = expiration ''' % (
                    asn, description, ip, protocol, discovered, expiration, category, comments)
                # print "query = %s" % query
                self.cursor.execute(query)
